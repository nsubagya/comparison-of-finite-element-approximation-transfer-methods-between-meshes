...
%foward Euler and back diff method in space
%N-total time
...
function v2=backwardeulerFEMwave5(x,y,dt,tend,swch)
...
while t<=tend
    ...
    %             lambda=dt/H;
    %             M(length(M),length(M)-1:length(M))=M(length(M),length(M)-1:length(M))+[lambda*alpha/2,1];
    LargeVal=1.0e8;
    %Dirichlet BC
    M(1,1)=LargeVal;
