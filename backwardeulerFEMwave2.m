...
%backward Euler and centerd diff method in space
%N-total time
...
%Mass lumping (2)
%as an example use this "backwardeulerFEMwave2(0:0.5:10,0:0.25:10,5,1)"
%for non overlapping vertices use geny.m
function v2=backwardeulerFEMwave2(x,y,dt,tend,swch)
...
while t<=tend
    ...
    for i=2:length(y)-1
        ...
        lambda=dt/H;
        M(i,i-1:i+1)=M(i,i-1:i+1)+[-lambda*alpha/2,1,lambda*alpha/2];
        
    end
    %             H=y(2)-y(1);
    %             lambda=dt/H;
    %             M(1,1:2)=M(1,1:2)+[1,lambda*alpha/2];
    
    H=y(length(y))-y(length(y)-1);
    lambda=dt/H;
    M(length(y),length(y)-1:length(y))=M(length(y),length(y)-1:length(y))+[-lambda*alpha/2,1];
    LargeVal=1.0e8;
